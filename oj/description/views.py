...
from oj.description.forms import (ItemForm,ItemRelationForm,ItemRelationCombinationForm,
                                        ...
                                        DescriptionDetailForm,DescriptionForm
                                        )
from oj.description.models import (Item,ItemRelation,ItemRelationCombination,
                                         ...
                                         DescriptionDetailFormInfo,Description,
                                         DescriptionDetail
                                         )
...
def description_detail_add(request, description_type_id,description_id):
        ...
        for i in xrange(obj.multi_number):
            ...
            for item_in_combitition in item_relation_combination:            
                ...
                for j in xrange(item_in_combitition.combination_number):
                    detail_object = DescriptionDetailFormInfo(
                                                      relation_show_order=relation_show_number,
                                                      item_show_order= item_show_number,
                                                      item_id = item.id,
                                                      relation_id = item_relation_object.id,
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            id = form.save(description_type_id = description_type_id,description_id=description_id)
            if id is not None:
                pass
                data = {'description_type_id':description_type_id,'page':1}
                return HttpResponseRedirect(reverse('description_list', kwargs=data)) 
    else:
...
def description_add(request): 
    session = Session()
    description_type_objects = session.query(DescriptionType).all()
    
    session.close()
    
    if description_type_objects is None:
        raise Http404
    
    description_type_choice = [(obj.id,obj.title) for obj in description_type_objects]  
   
    initial={"description_type_choice":description_type_choice,}
            
    if request.method == 'POST':
        form = DescriptionForm(request.POST,initial=initial,auto_id=False)
        if form.is_valid():
            ...
            return HttpResponseRedirect(reverse('description_detail_add', kwargs=data)) 
    else:
        form = DescriptionForm(initial=initial,auto_id=False)
        
    data = {'form': form,}
    
    return render_to_response("description/description_add.html", data, context_instance=RequestContext(request)) 

def description_show(request,description_id):
    session = Session()
    description_object = session.query(Description).get(description_id)
    
    description_detail_objects = session.query(DescriptionDetail).\
    filter(DescriptionDetail.description_id == description_id).\
    order_by(DescriptionDetail.show_order)
    if description_object is None or description_detail_objects is None:
        raise Http404
    
    relation_show_order = description_detail_objects[0].relation_show_order
    item_id = description_detail_objects[0].items_id    
    item_object = session.query(Item).get(item_id)

    html_string = '<div class="relation_show_order %s">' % (description_detail_objects[0].relation_show_order)
    change_relation = False

    html_string += ('<span class="group_item_id %s">%s&nbsp:&nbsp' % (item_id,item_object.title)) \
    + description_detail_objects[0].content + '</span>'
    
    for obj in description_detail_objects[1:]:
        
        if relation_show_order != obj.relation_show_order:
            relation_show_order = obj.relation_show_order
            change_relation = True
            item_object = session.query(Item).get(obj.items_id)
            html_string += '</div><div class="relation_show_order %s"><span class="group_item_id %s">%s&nbsp:&nbsp' \
            % (obj.relation_show_order,obj.items_id,item_object.title)
        else:
            change_relation = False
        
        if change_relation == False:
            if item_id != obj.items_id:
                item_object = session.query(Item).get(obj.items_id)

                html_string += '</span><br /><span class="group_item_id %s">%s&nbsp:&nbsp' % (obj.items_id,item_object.title)
 
            else:
                html_string += '&nbsp'

        item_id = obj.items_id
        
                 
        html_string += ('<span class = "item_id %s">' % (item_id) ) + obj.content + '</span>' 
    html_string += '</div>'        

    session.close()
    data = {"html_string":html_string}
    return render_to_response("description/description_show.html", data, context_instance=RequestContext(request)) 


def description_list(request,description_type_id , page=1):
    session = Session()
    description_objects = session.query(Description).\
    filter(Description.description_type_id == description_type_id).\
    order_by(Description.id)

    session.close()
    
    paginator = Paginator(description_objects, settings.METAS_PER_PAGE)
    
    try:
        objects = paginator.page(description_objects)
    except (EmptyPage, InvalidPage):
        objects = paginator.page(paginator.num_pages)
    data = {"objects": objects}
    return render_to_response("description/description_list.html", data,
                              context_instance=RequestContext(request))
